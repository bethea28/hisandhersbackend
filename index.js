...
const resolvers = {
	Query: {
		...
		search: (parent, { term, status }) => {
			...
			const byTerm = t => item => t.toLowerCase() === item.id.substr(0, t.length).toLowerCase();

			const byStatus = status => item => status.toLowerCase() === item.status.toLowerCase();
	...
	Trail: {
		accessedByLifts: parent => parent.lift.map(id => lifts.find(l => id === l.id)),
	},
